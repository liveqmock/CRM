...
public class AuthInterceptor implements HandlerInterceptor {
	...
	private boolean hasMenuAuth(HttpServletRequest request){
		...
		for (TSFunction function : functionList) {
			//update-begin--Author:zhangdaihao  Date:20140802 for：[bugfree号]菜单路径名字重复的问题--------------------
			if (function.getFunctionUrl() != null && (function.getFunctionUrl().startsWith(requestPath+"&")||function.getFunctionUrl().equals(requestPath))) {
			//update-end--Author:zhangdaihao  Date:20140802 for：[bugfree号]菜单路径名字重复的问题----------------------
				bMgrUrl = true;
				break;
			}
		}
